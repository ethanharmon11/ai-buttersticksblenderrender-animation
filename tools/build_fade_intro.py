...
// out: opacity only, mark and lining together. No scale on the way out —
// moving the letterforms while they fade re-rasterizes their edges every
// frame, which reads as the mark shaking just as it leaves.
// Nothing follows: the timeline ends on the last visible frame so the page
// can take over immediately instead of sitting on an empty cream screen.
tl.addLabel("out", 4.0)
  .to(["#wordmark", "#lining"], {{ autoAlpha: 0, duration: 0.7,
      ease: "sine.in" }}, "out");
...
tl.eventCallback("onComplete", () => {
  // The mark has just finished fading, so the screen is already empty
  // cream. Fading it again would only delay the site. Signal and vanish.
